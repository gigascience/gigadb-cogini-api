	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('view','checkDOIExist' , 'aSetSortCookies','next'),
				'users'=>array('*'),
			),
...
EO_MAIL;
        ...
        public function actionNext(){
            if(isset($_GET['doi'])){
                ...
                $result = Dataset::model()->findAllBySql("select id from dataset where identifier='".$doi."';");
                $cur_id = $result[0]->id;
              // $this->renderPartial('dataset/'.$cur_id);
                $result = Dataset::model()->findAllBySql("select id from dataset order by id desc limit 1;");
                $max_id = $result[0]->id;
                if($cur_id != $max_id)
                    $next_id=$cur_id+1;
                
                $result = Dataset::model()->findAllBySql("SELECT identifier from dataset where id=".$next_id);
                 $next_doi=$result[0]->identifier; 
                 $this->redirect('/dataset/'.$next_doi);
            }
        }
