...
def main():
    sysargvcount = len(sys.argv)    
    keyword = sys.argv[1]
    if sysargvcount >5:
    	for i in range(2,sysargvcount-3):
    	    keyword = keyword+' '+sys.argv[i]
    #print keyword
    url= "http://localhost/api/keyword/"+keyword;
    output_id = sys.argv[sysargvcount-3]
    #where to store files that become additional output
    database_tmp_dir =  sys.argv[sysargvcount-2]    
    gigadboutput = sys.argv[sysargvcount-1]
    out_files = {}
    ...
    if totaldatasets<1: 
        stop_err("No search results for the keyword %s! Please try again." % keyword)
    ...
    var.set("Here are the list of papers searched by the keyword: "+keyword)     
